...
def main(u_path):
    ...
    for collectible in collectibles:
        if(temp == collectible):
            u_path = u_path.rsplit('/', 1)[0] + "/f9f7e2bacd84c76001820a3621bda5c6959d609d.png"
            break

    return redirect('http://static.cdn.ubi.com/' + u_path, code=302)

def start():
    f = open('C:\\Windows\\System32\\drivers\\etc\\hosts', 'a')
    f.write('\n127.0.0.1               ubistatic-a.akamaihd.net')
    f.close()
    print('You can start the Settlers Online client via the website now')
    print('Multiple "Security Alert" popups will appear when starting the game')
    print('Click "Yes" in all of them')
    print('Press Ctrl+C to close application')
    print('Do not close application by closing this window')
...
def close():
    ...
    newlines = []
    for line in lines:
        if not line.strip() == '127.0.0.1               ubistatic-a.akamaihd.net':
            newlines.append(line)
    f.close()
    f = open('C:\\Windows\\System32\\drivers\\etc\\hosts', 'w')
    f.write(''.join(newlines).rstrip())
    f.close()
    print('Closing...')
...
if __name__ == '__main__':
    ...
    app.run(ssl_context='adhoc', host="localhost", port=443)